...
Two modes via --mode {backfill,forward}:
  ...
  backfill (one-time): for each active ticker, enumerate quarters from
    BACKFILL_START (default 2021Q1, last 5 years) through the current quarter.
    Skip any (symbol, quarter) that already exists in bronze. Wall time at
    75 calls/min: ~30K calls × 0.8s ≈ 7 hours for 1,500 tickers × 20 quarters.
...
def _all_backfill_quarters() -> List[str]:
    """Return ['2021Q1', '2021Q2', ..., current_quarter]."""
    today = date.today()
    end_year, end_q = today.year, (today.month - 1) // 3 + 1
    out: List[str] = []
    y, q = BACKFILL_START_YEAR, BACKFILL_START_Q
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="backfill (one-time) enumerates all quarters since 2021Q1; "
             "forward (default) reads recent report_dates from silver fact_earnings.",
    )
    ...
    if args.mode == "backfill":
        ...
        quarters = _all_backfill_quarters()
